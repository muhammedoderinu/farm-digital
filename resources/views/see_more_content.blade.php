...
<nav class="relative px-4 py-4 flex justify-between items-center bg-black">
    ...
    <div class="flex mt-5 py-5 flex-col divide-y dark:divide-gray-700">
                    @foreach($posts as $post)
                    
                    <div class="flex mb-3 px-1 md:py-4" onclick="getContent('{{$post->id}}')">
                        <img  alt="" class="flex-shrink-0 object-cover w-20 h-20 mr-4 dark:bg-gray-500" src=" {{$post->media->url}}">
                        <div class="flex flex-col flex-grow">
                            <a rel="noopener noreferrer" href="#" class="font-serif hover:underline">{{$post->title}}</a>
                            <p class="mt-auto text-xs dark:text-gray-400">{{date('M d, Y',$post->created_at->timestamp)}}
                                <a rel="noopener noreferrer" href="#" class="block dark:text-blue-400 lg:ml-2 lg:inline hover:underline">{{$post->category->name}}</a>
                            </p>
                        </div>
                    </div>
                    @endforeach
